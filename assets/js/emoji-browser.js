...
const words = {
  ru: { title: 'Эмодзи NekoChat', search: 'Поиск emoji…', all: 'Все', found: 'Найдено', groups: 'Категории emoji' },
  en: { title: 'NekoChat Emoji', search: 'Search emoji…', all: 'All', found: 'Found', groups: 'Emoji categories' }
};
const groupNames = {
...
let language = 'ru'; let selectedGroup = 'all';
...
function applyText() { document.documentElement.lang = language; document.title = t('title'); $('.xp-title').textContent = t('title'); $('#emoji-search').placeholder = t('search'); $('#emoji-groups').setAttribute('aria-label', t('groups')); render(); }
...
$('#emoji-grid').onclick = event => { const emoji = event.target.closest('[data-emoji]')?.dataset.emoji; if (emoji) controls.selectEmoji(emoji); };
